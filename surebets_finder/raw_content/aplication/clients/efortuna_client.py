...
class EFortunaClient(IWebClient):
    MAIN_HTML_DIV_ID = "sport-events-list-content"
    ...
    def _make_request(self, url: str) -> str:
        try:
            ...
            self._logger.exception(f"[efortuna.pl] HTTP Error! {str(e)}")
            raise RequestError("HTTP Error!") from e
        except requests.exceptions.ConnectionError as e:
            self._logger.exception(f"[efortuna.pl] Connection Error! {str(e)}")
            ...
            self._logger.exception(f"[efortuna.pl] Request Error! {str(e)}")
            raise RequestError("Request Error!") from e

    def _extract_information(self, page_content: str) -> str:
        soup = BeautifulSoup(page_content, "html.parser")

        return str(soup.find(id=self.MAIN_HTML_DIV_ID))
