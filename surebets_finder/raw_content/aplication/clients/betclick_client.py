...
@inject
class BetClickClient(IWebClient):
    ...
    def _make_request(self, url: str) -> str:
        try:
            ...
            self._logger.exception(f"[betclic.pl] HTTP Error! {str(e)}")
            raise RequestError("HTTP Error!") from e
        except requests.exceptions.ConnectionError as e:
            self._logger.exception(f"[betclic.pl] Connection Error! {str(e)}")
            raise RequestError("Connection Error!") from e
        except requests.exceptions.Timeout as e:
            self._logger.exception(f"[betclic.pl] Timeout Error! {str(e)}")
            raise RequestError("Timeout Error!") from e
        except requests.exceptions.RequestException as e:
            self._logger.exception(f"[betclic.pl] Request Error! {str(e)}")
